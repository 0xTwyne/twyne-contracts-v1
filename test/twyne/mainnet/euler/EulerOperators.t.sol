...
contract EulerOperators is EulerTestBase {
    ...
    function test_e_LeverageOperator_EulerFL() public noGasMetering {
...

        {
            ...
            // Check that controller is disabled before operation
            assertFalse(evc.isControllerEnabled(address(this), address(eulerUSDC)), "Controller should be disabled before operation");

            evc.batch(items);

            // Check that controller is disabled after operation
            assertFalse(evc.isControllerEnabled(address(this), address(eulerUSDC)), "Controller should be disabled after operation");
