...
contract EulerFrontendTests is EulerTestBase {
    ...
    function test_e_batchOpenBorrowSim() public noGasMetering {
        ...
        items[0] = IEVC.BatchItem({
            ...
            value: 0,
            data: abi.encodeCall(collateralVaultFactory.createCollateralVault, (VaultType.EULER_V2, eulerYzPP, eulerUSDT, twyneLiqLTV, address(0)))
        });
        vm.startPrank(alice);
        ...
        IERC20(YzPP).approve(permit2, type(uint).max);
    ...
    //     (uint externalCollateralValueScaledByLiqLTV, ) = IEVault(eulerUSDT).accountLiquidity(address(user_collateral_vault), true);
    ...
    //         user_collateral_vault.totalAssetsDepositedOrReserved() - user_collateral_vault.maxRelease(), user_collateral_vault.asset(), IEVault(eeYzPP_intermediate_vault).unitOfAccount());
    ...
    //         user_collateral_vault.totalAssetsDepositedOrReserved() * uint(IEVault(user_collateral_vault.targetVault()).LTVBorrow(eulerYzPP)) / MAXFACTOR,
    //         eulerYzPP,
    //         USD
    //     );
    ...
    //     uint maxBorrowAmount = maxBorrowValueTwyneLimit / eulerOnChain.getQuote(1, USDT, USD);
    //     console2.log("maxBorrowAmount in USDT", maxBorrowAmount);
    ...
    function test_e_frontend_depositUnderlyingViaTwyneEVC() external noGasMetering {
        // Bob convert YzPP from eulerYzPP
        vm.startPrank(bob);
        // First, approve permit2 to allow permit2 usage in batch
        IERC20(YzPP).approve(permit2, type(uint).max);
        Permit2ECDSASigner permit2Signer = new Permit2ECDSASigner(address(permit2));
        ...
        items[2] = IEVC.BatchItem({
            ...
            onBehalfOfAccount: bob,
            value: 0,
            data: abi.encodeCall(IERC20(YzPP).approve, (eulerYzPP, type(uint).max))
        });
        items[3] = IEVC.BatchItem({
            targetContract: eulerYzPP,
            onBehalfOfAccount: bob,
            value: 0,
            data: abi.encodeCall(IEVault(eulerYzPP).deposit, (CREDIT_LP_AMOUNT, bob))
        });
        console2.log(IEVault(eulerYzPP).balanceOf(bob));
        evc.batch(items);
        vm.stopPrank();
    ...
    // Not valid on plasma as we are not testing with native xpl token for now
    // function test_e_frontend_depositETHViaTwyneEVC() external noGasMetering {
    //     // Bob converts ETH to eulerYzPP
    //     vm.startPrank(bob);
    //     IEVC.BatchItem[] memory items = new IEVC.BatchItem[](3);
    ...
    //         onBehalfOfAccount: bob,
    //         value: 0,
    //         data: abi.encodeCall(IEVault(eulerYzPP).deposit, (bal, bob))
    //     });
    //     console2.log(IEVault(eulerYzPP).balanceOf(bob));
    ...
    // Will fail because Morpho is not available on Plasma
    ...
    //     // Now test deleverage
    //     uint flashloanAmount = alice_collateral_vault.totalAssetsDepositedOrReserved() - alice_collateral_vault.maxRelease(); // YzPP flashloan for deleverage
    ...
    //     uint minAmountOutUSDT = alice_collateral_vault.maxRepay();
    //     deal(USDT, eulerSwapper, minAmountOutUSDT + 10);

    //     // Create deleverage swap data
    //     bytes[] memory deleverageMulticallData = new bytes[](1);
    //     {
    //         bytes memory deleverageSwapData = abi.encodeCall(MockSwapper.swap, (YzPP, USDT, flashloanAmount, minAmountOutUSDT, address(deleverageOperator)));
